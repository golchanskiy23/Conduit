...
type mockSubmitter struct {
	lastJob  *scheduler.Item
	lastDeps []string
	err      error
}

func (m *mockSubmitter) Submit(job *scheduler.Item, deps []string) error {
	m.lastJob = job
	m.lastDeps = deps
	return m.err
}
...
func TestEnqueueJob_Success(t *testing.T) {
	...
	if sub.lastJob.Priority != scheduler.PriorityHigh {
...
func TestEnqueueJob_GeneratesJobID(t *testing.T) {
	...
	rr := httptest.NewRecorder()
	h.EnqueueJob(rr, makeRequest(t, EnqueueRequest{Priority: scheduler.PriorityNormal}))
...
func TestEnqueueJob_CollisionReturns409(t *testing.T) {
	sub := &mockSubmitter{err: scheduler.ErrAlreadyExists}
...
func TestEnqueueJob_NoDeps(t *testing.T) {
	...
	rr := httptest.NewRecorder()
	h.EnqueueJob(rr, makeRequest(t, EnqueueRequest{Priority: scheduler.PriorityLow}))
